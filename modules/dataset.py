...
class DeepEyeNet_GPT(Dataset):
    def __init__(self, args, tokenizer, keywords_list, split, transform=None):
        ...
        # Define special token IDs (GPT-2 specific tokens)
        ...
        self.sep_token_id = self.tokenizer.encode("<|sep|>")[0]  # Custom separator for keywords
    ...
    def __getitem__(self, idx):
        ...
        # --- Tokenize clinical description ---
        desc_tokens = self.tokenizer.encode(clinical_desc, truncation=True, max_length=self.max_length)
        ...
        # --- Encode keywords with <SEP> separator ---
        if keywords:
            raw_keywords = f" <|sep|> ".join([kw.strip() for kw in keywords.split(',') if kw.strip()])  # Join keywords with <SEP>
        else:
            raw_keywords = "<|sep|>"  # Handle empty keywords case

        keyword_tokens = self.tokenizer.encode(raw_keywords, truncation=True, max_length=self.max_length)
        keyword_tokens = torch.tensor(keyword_tokens, dtype=torch.long)
