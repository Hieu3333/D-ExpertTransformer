...
class ExpertTransformer(nn.Module):
    ...
    @torch.no_grad()
    def generate_v2(self, images, gt_keywords=None):
        device = self.device
        batch_size = images.size(0)
        beam_width = self.beam_width
        max_len = self.max_gen
        ...
        # Initialize beams (batch_size, beam_width, seq_len=1)
        sequences = torch.full((batch_size, beam_width, 1), bos_id, 
                            device=device, dtype=torch.long)
        
        # Log probabilities for each beam (batch_size, beam_width)
        log_probs = torch.zeros(batch_size, beam_width, device=device)
        
        # Track finished beams (batch_size, beam_width)
        finished = torch.zeros_like(log_probs, dtype=torch.bool)

        # Expand inputs for beam search
        if images.dim() == 4:  # (batch, channels, H, W)
            images = images.unsqueeze(1).expand(-1, beam_width, *[-1]*3)
            images = images.reshape(batch_size * beam_width, *images.shape[2:])
        
        # Expand gt_keywords if provided (assuming shape [batch_size, ...])
        if gt_keywords is not None:
            gt_keywords = gt_keywords.unsqueeze(1).expand(-1, beam_width, *[-1]*(gt_keywords.dim()-1))
            gt_keywords = gt_keywords.reshape(batch_size * beam_width, *gt_keywords.shape[2:])

        for step in range(1, max_len):
            # Flatten sequences for processing (batch*beam, seq_len)
            flat_sequences = sequences.view(batch_size * beam_width, -1)
            
            # Get logits (batch*beam, seq_len, vocab_size)
            logits, _ = self(images, flat_sequences, gt_keywords)
            logits = logits[:, -1, :] / self.temperature
            probs = F.log_softmax(logits, dim=-1).view(batch_size, beam_width, -1)

            # Expand candidates (batch, beam, vocab)
            current_scores = log_probs.unsqueeze(-1) + probs
            
            # For first step, we only have one beam (the start token)
            if step == 1:
                current_scores = current_scores[:, 0, :].unsqueeze(1)

            # Flatten to (batch, beam*vocab) and select top-k
            vocab_size = probs.size(-1)
            current_scores_flat = current_scores.view(batch_size, -1)
            
            # Get top-k candidates (k = beam_width)
            top_scores, top_indices = torch.topk(
                current_scores_flat, k=beam_width, dim=-1
            )
            
            # Determine which beam and token generated the candidates
            beam_indices = top_indices // vocab_size  # Original beam index
            token_indices = top_indices % vocab_size  # Selected token index

            # Update sequences
            new_sequences = []
            for batch_idx in range(batch_size):
                # Get sequences from the beams that survived
                batch_beams = sequences[batch_idx][beam_indices[batch_idx]]
                
                # Append new tokens
                new_tokens = token_indices[batch_idx].unsqueeze(1)
                new_seq = torch.cat([batch_beams, new_tokens], dim=1)
            ...
            new_finished = (token_indices == eos_id)
            finished = torch.gather(finished, 1, beam_indices) | new_finished
        ...
        best_indices = log_probs.argmax(dim=-1)  # (batch_size,)
        ...
        for seq in best_sequences:
            ...
            final_texts.append(self.tokenizer.decode(tokens))
